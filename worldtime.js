...
function init() {
    ...
    if (searchInput) searchInput.addEventListener("input", filterCities);
...
function updateAllClocks() {
    ...
    // 更新北京时间（使用 3D 翻页逻辑）
    updateFlipClock(now, "Asia/Shanghai");
...
// --- 新增：主时钟翻页控制逻辑 ---
function updateFlipClock(baseTime, timeZone) {
    // 利用 en-GB 格式化确保强制按 24 小时制输出 "HH:mm:ss"
    ...
    // 触发单体翻页
    flipUnit('flip-hour', h, 'hour');
    flipUnit('flip-minute', m, 'minute');
    flipUnit('flip-second', s, 'second');
...
function flipUnit(unitId, newValue, timePart) {
    if (previousTime[timePart] === newValue) return; // 数值没变，跳过

    const unitEl = document.getElementById(unitId);
    if (!unitEl) return;

    const topStatic = unitEl.querySelector('.top-static');
    const bottomStatic = unitEl.querySelector('.bottom-static');
    const flapFront = unitEl.querySelector('.flap-front');
    const flapBack = unitEl.querySelector('.flap-back');

    const oldValue = previousTime[timePart] || newValue;

    // 1. 设置动画初始状态：底板为新数字，背面的翻页也是新数字，准备露出
    topStatic.textContent = newValue;
    flapBack.textContent = newValue;
    
    // 2. 当前旧数字保持在下半底板和翻页正面
    bottomStatic.textContent = oldValue;
    flapFront.textContent = oldValue;

    // 3. 移除类名并利用 offsetWidth 强制重绘，确保动画可以重复触发
    unitEl.classList.remove('flipping');
    void unitEl.offsetWidth;
    unitEl.classList.add('flipping');

    // 4. 更新记录
    previousTime[timePart] = newValue;

    // 5. 动画完成后（0.6s），将所有面统一替换为新数字，移除动画状态
    clearTimeout(flipTimeouts[timePart]);
    flipTimeouts[timePart] = setTimeout(() => {
